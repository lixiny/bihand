...
class NetBiHand(nn.Module):
    def __init__(
        ...
        cprint("params upstream: {:.3f}M".format(param_count(self.upstream)), "green", attrs=["bold"])
        if "sik" in net_modules:
            self.siknet = SIKNet()
            cprint("params siknet: {:.3f}M".format(param_count(self.siknet)), "blue", attrs=["bold"])
    ...
    def load_checkpoints(
        ...
        if ckp_seednet and os.path.isfile(ckp_seednet) and "seed" in self.net_modules:
            misc.load_checkpoint(self.upstream.seednet, ckp_seednet)

        if ckp_liftnet and os.path.isfile(ckp_liftnet) and "lift" in self.net_modules:
            misc.load_checkpoint(self.upstream.liftnet, ckp_liftnet)

        if ckp_siknet and os.path.isfile(ckp_siknet) and "sik" in self.net_modules:
            misc.load_checkpoint(self.siknet, ckp_siknet)
